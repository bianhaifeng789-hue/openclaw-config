...
// Types defined inline for standalone use

// MCP服务器审批状态
interface MCPServerApprovalState {
...
// 单例状态
let state: MCPServerApprovalState = {
 ...
 * 检查服务器是否需要审批
 */
export function needsApproval(serverName: string, capabilities: string[]): boolean {
  // 已批准的服务器不需要重复审批
 ...
 * 批准服务器
 */
export function approveServer(serverId: string): boolean {
 ...
 * 拒绝服务器
 */
export function denyServer(serverId: string): boolean {
  const approval = state.pendingApprovals.get(serverId);
  if (!approval) {
    return false;
  }
 ...
 * 自动批准安全服务器
 */
export function autoApproveSafe(serverId: string): boolean {
  const approval = state.pendingApprovals.get(serverId);
  if (!approval) {
    return false;
  }
 ...
 * 检查服务器是否已批准
 */
export function isApproved(serverName: string): boolean {
 ...
 * 检查服务器是否已拒绝
 */
export function isDenied(serverName: string): boolean {
 ...
 * 获取统计信息
 */
export function getStats(): MCPServerApprovalState['stats'] {
  return { ...state.stats };
}
 ...
 * 生成飞书审批卡片
 */
export function generateApprovalCard(approval: PendingApproval): object {
