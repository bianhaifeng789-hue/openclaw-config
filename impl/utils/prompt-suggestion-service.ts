...
export interface PromptSuggestionConfig {
  ...
  maxSuggestions: number        // 最大建议数（默认 3）
  minAssistantTurns: number     // 最小 assistant turns（默认 2）
  cooldownMinutes: number       // 冷却期（默认 30）
  maxPerDay: number             // 每天最大建议数（默认 5）
}

export interface Suggestion {
  prompt: string                // 建议提示
  variant: PromptVariant        // 变体类型
...
export interface SuggestionState {
  currentSuggestion: Suggestion | null   // 当前建议
  suggestionHistory: Suggestion[]        // 建议历史
  totalGenerated: number                 // 总生成数
  totalAccepted: number                  // 总接受数
  todayCount: number                     // 今日建议数
  lastGeneratedAt: number                // 上次生成时间
}

export interface SuppressReason {
  reason: string
  detail?: string
}
...
const DEFAULT_CONFIG: PromptSuggestionConfig = {
  enabled: true,
  maxSuggestions: 3,
  minAssistantTurns: 2,
  cooldownMinutes: 30,
 ...
 */
export function checkMinAssistantTurns(messages: Array<{ role: string }>): boolean {
 ...
 * 用户忽略建议
 */
export function ignoreSuggestion(): void {
...
export function getState(): SuggestionState {
  return {
...
export function getConfig(): PromptSuggestionConfig {
  return DEFAULT_CONFIG
}

export function getCurrentSuggestion(): Suggestion | null {
 ...
 * 构建飞书建议卡片
 */
export function createSuggestionCard(suggestion: Suggestion | null): {
  title: string
  content: string
  buttons?: string[]
} {
 ...
 * Heartbeat 检查点
 */
export async function checkPromptSuggestionForHeartbeat(
  messages: Array<{ role: string; content: string }>,
): Promise<{
