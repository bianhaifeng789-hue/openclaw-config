...
let _config: TokenEstimationConfig = {
...
let _lastEstimation: TokenEstimationResult | null = null

export function getConfig(): TokenEstimationConfig {
  return { ..._config }
}

export function setConfig(config: Partial<TokenEstimationConfig>): void {
  _config = { ..._config, ...config }
}

export function getLastEstimation(): TokenEstimationResult | null {
 ...
 * 检测语言类型
 */
function detectLanguageType(text: string): 'english' | 'chinese' | 'code' | 'mixed' {
...
export function calculateBudget(
  usedTokens: number,
  maxContext: number = MAX_CONTEXT_TOKENS
): TokenBudget {
 ...
 * 生成预算警告
 */
export function getBudgetWarning(budget: TokenBudget): string {
...
export function createTokenEstimationHook() {
  return {
    ...
    afterResponse: (usage: { input_tokens: number; output_tokens: number }) => {
      // 更新实际 token 使用
...
export function getSystemStats(): {
  ...
  lastEstimation: TokenEstimationResult | null
