...
/**
 * Startup Profiler Pattern - 启动分析器
 * 
 * Source: Claude Code utils/startupProfiler.ts
 * Pattern: startup profiling + phases + timing + bottlenecks
 */
...
interface StartupPhase {
  ...
  children: StartupPhase[]
}

class StartupProfiler {
  private phases = new Map<string, StartupPhase>()
  private currentPhase: StartupPhase | null = null
  private phaseStack: StartupPhase[] = []
  private rootPhase: StartupPhase | null = null
  private startTime = 0
   ...
   * End profiling
   */
  end(): StartupPhase | null {
    if (!this.rootPhase) return null
   ...
   * Get phase
   */
  getPhase(name: string): StartupPhase | undefined {
   ...
   * Find bottlenecks
   */
  findBottlenecks(thresholdMs: number = 100): StartupPhase[] {
    const bottlenecks: StartupPhase[] = []
   ...
   */
  formatReport(): string {
    if (!this.rootPhase) return 'No profiling data'
   ...
   * Format phase tree
   */
  private formatPhaseTree(phase: StartupPhase, lines: string[], indent: number): void {
