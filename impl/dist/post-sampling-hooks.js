 ...
 */
export function registerPostSamplingHook(hook, metadata) {
    const hookMetadata = {
 ...
 * 启用或禁用指定钩子
 */
export function setHookEnabled(name, enabled) {
 ...
 */
export async function executePostSamplingHooks(context) {
 ...
 */
export function createSessionMemoryHook(shouldExtract, extract) {
    return async (context) => {
        // 只在主线程或飞书消息时运行
        const validSources = ['repl_main_thread', 'feishu_message', 'heartbeat'];
 ...
 */
export function createCacheSaveHook(saveParams) {
    return (context) => {
        // 保存 cache params
        saveParams(context);
    };
 ...
 * - 避免重复询问
 */
export function createPermissionTrackingHook(trackDecisions) {
    return (context) => {
        // 只在主线程追踪
 ...
 * 在达到条件时触发记忆整合
 */
export function createAutoDreamHook(shouldDream, triggerDream) {
    return async (context) => {
        // 只在主线程检查
        if (context.querySource !== 'repl_main_thread') {
            return;
...
export function createDefaultHooks() {
    // 1. Cache 参数保存（最高优先级）
    registerPostSamplingHook(createCacheSaveHook((context) => {
        // 保存 cache params
    ...
    registerPostSamplingHook(createPermissionTrackingHook((context) => {
        // 追踪权限决策
    ...
    registerPostSamplingHook(createSessionMemoryHook((context) => {
        // 检查是否需要提取
    ...
    registerPostSamplingHook(createAutoDreamHook((context) => {
        // 检查是否需要 dream
        return false; // 暂时禁用
    }, async () => {
...
//# sourceMappingURL=post-sampling-hooks.js.map