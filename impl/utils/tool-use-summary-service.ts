...
interface SummaryState {
  summaries: string[]
  toolStats: Map<string, { count: number; success: number; fail: number }>
  stats: {
    totalTools: number
...
// 摘要模板
const SUMMARY_TEMPLATES: Record<string, (tools: ToolInfo[]) => string> = {
...
export function generateSummary(params: SummaryParams): string {
  ...
  // 按工具名分组
  const byTool: Record<string, ToolInfo[]> = {}
  for (const tool of tools) {
    const baseName = getBaseToolName(tool.name)
    if (!byTool[baseName]) byTool[baseName] = []
  ...
  for (const tool of tools) {
    ...
    if (tool.success) state.stats.successfulTools++
    else state.stats.failedTools++
  }
 ...
 * 获取最近摘要
 */
export function getRecentSummaries(count: number = 5): string[] {
 ...
 * 获取工具统计
 */
export function getToolStats(): Map<string, { count: number; success: number; fail: number }> {
 ...
 * 获取总体统计
 */
export function getStats(): SummaryState['stats'] {
  return { ...state.stats }
}
 ...
 * 生成飞书摘要卡片
 */
export function generateSummaryCard(): object {
