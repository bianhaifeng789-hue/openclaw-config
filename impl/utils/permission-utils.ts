...
} from './permission-decision-tracker'

import {
  postSamplingHooksSystem,
  registerPostSamplingHook,
  createPermissionTrackingHook,
} from './post-sampling-hooks'
...
/**
 * Combined Permission Analysis - 综合权限分析
 */
export interface CombinedPermissionAnalysis {
  /** 原始命令 */
  command: string
  /** 命令分类 */
  classification: ClassificationResult
  /** 危险模式分析 */
  dangerAnalysis: CommandAnalysisResult
  /** 用户偏好学习 */
  preference: LearnedPreference | null
  /** 综合决策建议 */
  recommendation: {
    action: 'auto_allow' | 'auto_deny' | 'ask_user' | 'forbidden'
    reason: string
    confidence: number
  }
  /** 安全建议 */
 ...
 * @returns 综合分析结果
 */
export function analyzeCommandForPermission(
  command: string
): CombinedPermissionAnalysis {
  // 1. 分类命令
...
function determineRecommendation(
  ...
  dangerAnalysis: CommandAnalysisResult,
  preference: LearnedPreference | null
): CombinedPermissionAnalysis['recommendation'] {
  // 极危险操作 → forbidden
...
/**
 * Permission Decision Flow - 权限决策流程
 */
export interface PermissionDecisionFlow {
  /** 步骤名称 */
  step: string
  /** 检查结果 */
 ...
 * @returns 决策流程记录
 */
export function runPermissionDecisionFlow(
  command: string
): {
  flows: PermissionDecisionFlow[]
  finalDecision: PermissionDecision
  analysis: CombinedPermissionAnalysis
} {
  const flows: PermissionDecisionFlow[] = []
 ...
 * 创建飞书权限审批卡片
 */
export function createFeishuPermissionCard(
  analysis: CombinedPermissionAnalysis
