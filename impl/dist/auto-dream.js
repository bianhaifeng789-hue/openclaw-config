 ...
 */
import { readLastConsolidatedAt, tryAcquireConsolidationLock, rollbackConsolidationLock, } from './consolidation-lock.js';
import { buildConsolidationPromptWithSessions } from './consolidation-prompt.js';
import { runForkedAgent, createCacheSafeParams } from './forked-agent.js';
 ...
 */
export function initAutoDream(config) {
 ...
 */
export function setAutoDreamEnabled(enabled) {
    state.enabled = enabled;
}
/**
 * Get current AutoDream state.
 */
export function getAutoDreamState() {
    return { ...state };
}
/**
 ...
 */
export async function runAutoDream(memoryRoot, context) {
    if (!state.enabled) {
        return;
    }
    ...
    try {
        ...
        console.error(`[AutoDream] readLastConsolidatedAt failed: ${e.message}`);
        return;
    }
    ...
    if (sinceScanMs < SESSION_SCAN_INTERVAL_MS) {
        console.log(`[AutoDream] Scan throttle: last scan was ${Math.round(sinceScanMs / 1000)}s ago`);
        return;
    }
    ...
    if (sessionIds.length < cfg.minSessions) {
        console.log(`[AutoDream] Session gate not passed: ${sessionIds.length} < ${cfg.minSessions}`);
        return;
    }
    // --- Lock ---
    let priorMtime;
    try {
        ...
        console.error(`[AutoDream] Lock acquire failed: ${e.message}`);
        return;
    }
    if (priorMtime === null) {
    ...
    try {
        ...
        console.log(`[AutoDream] Completed — cache: read=${result.totalUsage.cache_read_input_tokens} created=${result.totalUsage.cache_creation_input_tokens}`);
    }
    catch (e) {
        console.error(`[AutoDream] Fork failed: ${e.message}`);
 ...
 */
async function listSessionsTouchedSince(memoryRoot, sinceMs) {
 ...
 */
export async function triggerManualDream(memoryRoot) {
