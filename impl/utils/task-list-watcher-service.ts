 ...
 * 初始化任务目录
 */
export async function initTasksDir(tasksDir?: string): Promise<void> {
 ...
 * 认领任务
 */
export async function claimTask(taskId: string, agentId: string): Promise<boolean> {
  try {
 ...
 * 创建任务
 */
export async function createTask(prompt: string, priority?: 'high' | 'medium' | 'low'): Promise<Task> {
 ...
 * 获取统计
 */
export function getStats(): WatcherState['stats'] {
  return { ...state.stats }
}
 ...
 * 生成飞书任务卡片
 */
export function generateTaskWatcherCard(): object {
  return {
