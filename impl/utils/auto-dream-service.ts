...
export interface AutoDreamConfig {
  minHours: number       // 最小间隔小时数（默认 24）
  minSessions: number    // 最小会话数（默认 5）
  enabled: boolean       // 是否启用
}

export interface DreamState {
  lastConsolidatedAt: number  // 上次合并时间戳
  sessionCount: number        // 累计会话数
  isLocked: boolean           // 是否正在合并
  lockAcquiredBy: string | null  // 锁持有者
  filesTouched: string[]      // 修改的文件列表
  savedTokens: number         // 节省的 tokens
 ...
 */
export function checkTimeGate(config: AutoDreamConfig): boolean {
 ...
 */
export function checkSessionGate(config: AutoDreamConfig): boolean {
 ...
 * 锁门控：是否有其他进程正在合并
 */
export function checkLockGate(): boolean {
 ...
 * 扫描节流：避免频繁扫描
 */
export function checkScanThrottle(): boolean {
 ...
 */
export function shouldRunAutoDream(): { shouldRun: boolean; reason: string } {
  const config = getAutoDreamConfig()
 ...
 * 回滚锁（失败时）
 */
export async function rollbackLock(priorMtime: number): Promise<void> {
  state.isLocked = false
  state.lockAcquiredBy = null
 ...
 */
export async function listSessionsTouchedSince(since: number): Promise<string[]> {
...
export function getState(): DreamState {
  return { ...state }
}
 ...
 */
export async function checkAutoDreamForHeartbeat(): Promise<{
  shouldRun: boolean
  state: DreamState
  reason: string
}> {
 ...
 * 构建飞书合并完成卡片
 */
export function createDreamCompletionCard(result: DreamResult): {
  title: string
  content: string
} {
  if (!result.success) {
