...
export function getState(): MemdirState {
  return { ..._state }
}
...
export function setConfig(config: Partial<MemdirConfig>): void {
  _config = { ..._config, ...config }
}

export function getConfig(): MemdirConfig {
  return { ..._config }
}
...
export async function scanMemoryFiles(
  ...
  try {
    ...
    const mdFiles = entries.filter(
      f => typeof f === 'string' && 
           f.endsWith('.md') && 
           basename(f as string) !== 'MEMORY.md'
    ) as string[]
...
export async function findRelevantMemories(
  ...
  const selected = await selectRelevantMemoriesWithModel(
    query,
    candidates,
    signal,
    recentTools,
...
function heuristicSelect(
  ...
  const scored = memories.map(m => {
    ...
    // 类型偏好
    if (m.type === 'user') score += 15 // 用户偏好优先
    if (m.type === 'feedback') score += 10 // 反馈其次
    if (m.type === 'project') score += 8 // 项目信息
    
    // 新鲜度偏好
    const ageDays = memoryAgeDays(m.mtimeMs)
    if (ageDays === 0) score += 20 // 今天
    if (ageDays === 1) score += 15 // 昨天
    if (ageDays <= 7) score += 10 // 一周内
 ...
 * 估算记忆 tokens
 */
function estimateTokensForMemories(memories: MemoryHeader[]): number {
  ...
  return {
    ...
    beforePrompt: async (params: {
      ...
      const memories = await findRelevantMemories(
        ...
        undefined,
        params.recentTools,
        params.alreadySurfaced,
      )
      
      return { memories }
    },
    
    formatMemoriesForPrompt: (memories: RelevantMemory[]): string => {
