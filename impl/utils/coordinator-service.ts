...
export type CoordinatorMode = 'coordinator' | 'normal'
export type SessionMode = 'coordinator' | 'normal' | undefined

export interface CoordinatorConfig {
  enabled: boolean
  workerTools: Set<string>
...
export function getState(): CoordinatorState {
  return { ..._state }
}
...
export function setConfig(config: Partial<CoordinatorConfig>): void {
 ...
 * Worker 生成时调用
 */
export function onWorkerSpawned(): void {
 ...
 * 设置 Worker 可用工具
 */
export function setWorkerTools(tools: Set<string> | 'basic' | 'full'): void {
  if (tools === 'basic') {
 ...
 * 检查工具是否对 Worker 可用
 */
export function isToolAvailableForWorker(toolName: string): boolean {
  if (!isCoordinatorMode()) {
...
interface CoordinatorEvent {
  type: 'mode_switch' | 'worker_spawned' | 'session_start'
  timestamp: string
  data: Record<string, unknown>
}

const _events: CoordinatorEvent[] = []

function logModeSwitch(to: CoordinatorMode): void {
  _events.push({
    type: 'mode_switch',
...
export function logWorkerSpawned(agentId: string, description: string): void {
  onWorkerSpawned()
  _events.push({
    type: 'worker_spawned',
    timestamp: new Date().toISOString(),
...
export function getEvents(limit = 50): CoordinatorEvent[] {
