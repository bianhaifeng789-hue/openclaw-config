...
export interface ArgumentBinding {
  key: string
  value: string
  source: 'user' | 'system' | 'env' | 'file'
}

export interface ArgumentConfig {
  maxSubstitutions: number       // 最大替换次数（默认 100）
  escapeMode: 'none' | 'single' | 'double'  // 转义模式
  enabled: boolean
}
...
const DEFAULT_CONFIG: ArgumentConfig = {
  maxSubstitutions: 100,
  escapeMode: 'double',
  enabled: true,
}

let stats: ArgumentStats = {
...
let bindings: Map<string, ArgumentBinding> = new Map()
 ...
 * 设置参数绑定
 */
export function setBinding(key: string, value: string, source: ArgumentBinding['source']): void {
 ...
 * 获取参数绑定
 */
export function getBinding(key: string): ArgumentBinding | null {
 ...
 * 删除参数绑定
 */
export function removeBinding(key: string): boolean {
 ...
 * 执行参数替换
 */
export function substituteArguments(template: string): string {
  let result = template
  let substitutions = 0
 ...
 * 批量替换
 */
export function substituteAll(templates: string[]): string[] {
 ...
 * 检查模板中是否有未替换的参数
 */
export function hasUnresolvedArguments(template: string): string[] {
 ...
 * 设置系统默认参数
 */
export function setSystemDefaults(): void {
  // 常用系统参数
 ...
 * 从 JSON 文件加载参数
 */
export async function loadFromFile(filePath: string): Promise<number> {
  try {
...
export function getArgumentStats(): ArgumentStats & { config: ArgumentConfig; bindingCount: number } {
  return {
    ...stats,
