 ...
 */
export function hasMagicDocMarker(content) {
    return MAGIC_DOC_HEADER_PATTERN.test(content);
}
/**
 * Extract magic doc marker info.
 */
export function extractMagicDocMarker(filePath) {
    try {
        ...
        console.error(`[MagicDocs] Error reading ${filePath}: ${e.message}`);
        return null;
    }
}
/**
 ...
 */
export function scanForMagicDocs(directory) {
    const markers = [];
    try {
        ...
        console.error(`[MagicDocs] Scan error: ${e.message}`);
    }
    return markers;
}
/**
 ...
 */
export function buildMagicDocUpdatePrompt(marker) {
 ...
 */
export async function updateMagicDoc(marker, cacheSafeParams) {
    ...
    for (const msg of result.messages) {
        ...
        if (msg.content && typeof msg.content === 'string') {
            ...
            try {
                ...
                console.error(`[MagicDocs] Write failed: ${e.message}`);
            }
            break;
        }
    }
 ...
 */
export async function runMagicDocsMaintenance(workspaceRoot, cacheSafeParams) {
    if (!state.enabled) {
    ...
    for (const marker of markers) {
        try {
            ...
            console.error(`[MagicDocs] Update failed for ${marker.filePath}: ${e.message}`);
        }
    }
}
/**
 ...
 */
export function getMagicDocsState() {
    return { ...state };
}
/**
 * Enable/disable magic docs.
 */
export function setMagicDocsEnabled(enabled) {
    state.enabled = enabled;
}
/**
 * Quick check: does a file need magic doc maintenance?
 */
export function needsMagicDocMaintenance(filePath) {
    return hasMagicDocMarker(readFileSync(filePath, 'utf8'));
}
//# sourceMappingURL=magic-docs.js.map