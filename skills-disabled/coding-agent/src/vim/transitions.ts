...
import type { VimState, Operator } from './types.js'
...
export function processKey(key: string, ctx: VimTransitionContext): VimTransitionContext {
  const { state } = ctx
  
  if (state.mode === 'normal') return processNormalMode(key, ctx)
  if (state.mode === 'insert') return processInsertMode(key, ctx)
  if (state.mode === 'visual') return processVisualMode(key, ctx)
  return ctx
}

function processNormalMode(key: string, ctx: VimTransitionContext): VimTransitionContext {
  const { state, cursor, text, setText, setCursor } = ctx
  
  // Mode switches
  if (key === 'i') return { ...ctx, state: { ...state, mode: 'insert' } }
  if (key === 'a') {
    const newCursor = cursor.right()
    setCursor(newCursor)
    return { ...ctx, state: { ...state, mode: 'insert' }, cursor: newCursor }
  }
  if (key === 'v') return { ...ctx, state: { ...state, mode: 'visual', visualStart: cursor.offset } }
  ...
  if (state.pendingOperator) {
    ...
    if (key === 'i' || key === 'a') {
      const scope: 'inner' | 'around' = key === 'i' ? 'inner' : 'around'
    ...
    if (isMotion(key)) {
      ...
      setCursor(newCursor)
      const newMode: 'insert' | 'normal' = op === 'c' ? 'insert' : 'normal'
  ...
  // Start operator
  if (isOperator(key)) {
    return { ...ctx, state: { ...state, pendingOperator: key as Operator, count: state.count || 1 } }
  }
...
function processInsertMode(key: string, ctx: VimTransitionContext): VimTransitionContext {
...
function processVisualMode(key: string, ctx: VimTransitionContext): VimTransitionContext {
  const { state, cursor, setCursor } = ctx
  
  if (key === '<Esc>') return { ...ctx, state: { ...state, mode: 'normal', visualStart: null } }
