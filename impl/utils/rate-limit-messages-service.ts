...
// 限速类型
type RateLimitType = 'session' | 'daily' | 'weekly' | 'monthly' | 'custom'

// 限速状态
type RateLimitStatus = 'ok' | 'warning' | 'limited' | 'exhausted'
...
export function updateLimit(
  ...
  if (message) {
    ...
    if (message.severity === 'warning') state.stats.warningsSent++
    if (message.severity === 'error') state.stats.errorsSent++
    if (info.status === 'exhausted') state.stats.limitsReached++
  }
 ...
 * 生成限速消息
 */
function generateLimitMessage(info: RateLimitInfo): RateLimitMessage | null {
 ...
 * 获取限速类型名称
 */
function getLimitName(type: RateLimitType): string {
  switch (type) {
 ...
 * 获取所有限速信息
 */
export function getAllLimits(): Map<string, RateLimitInfo> {
 ...
 * 获取特定限速信息
 */
export function getLimit(key: string): RateLimitInfo | undefined {
 ...
 * 获取所有消息
 */
export function getMessages(): RateLimitMessage[] {
 ...
 * 获取最新消息
 */
export function getLatestMessage(): RateLimitMessage | undefined {
 ...
 * 获取统计信息
 */
export function getStats(): RateLimitState['stats'] {
  return { ...state.stats }
}
 ...
 * 生成飞书限速卡片
 */
export function generateRateLimitCard(): object {
