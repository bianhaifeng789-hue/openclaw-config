 ...
 */
export function getLockPath(memoryRoot: string): string {
 ...
 */
export async function readLastConsolidatedAt(memoryRoot: string): Promise<number> {
...
export async function tryAcquireConsolidationLock(
  ...
  if (mtimeMs !== undefined && Date.now() - mtimeMs < HOLDER_STALE_MS) {
    if (holderPid !== undefined && isProcessRunning(holderPid)) {
      console.log(
        `[ConsolidationLock] Lock held by live PID ${holderPid}`
      )
      return null
    }
...
export async function rollbackConsolidationLock(
  ...
  try {
    ...
    console.error(
      `[ConsolidationLock] Rollback failed: ${(e as Error).message}`
    )
  }
}
 ...
 */
export async function recordConsolidation(memoryRoot: string): Promise<void> {
  try {
    await mkdir(memoryRoot, { recursive: true })
    await writeFile(getLockPath(memoryRoot), String(process.pid))
  } catch (e) {
    console.error(
      `[ConsolidationLock] Record failed: ${(e as Error).message}`
    )
  }
}