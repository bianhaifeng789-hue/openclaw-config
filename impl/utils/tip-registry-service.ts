...
export interface Tip {
  id: string              // 提示 ID
  content: string         // 提示内容
  category: string        // 分类（new-user, advanced, feature, etc）
...
export interface TipHistory {
  tipId: string           // 提示 ID
  shownAt: number         // 显示时间
  sessionsSinceShow: number // 自显示后的会话数
}

export interface TipState {
  tips: Tip[]                     // 提示列表
  history: TipHistory[]           // 显示历史
  currentTip: Tip | null          // 当前提示
  totalShown: number              // 总显示数
  lastShownAt: number             // 上次显示时间
}

export interface TipConfig {
  enabled: boolean
  maxTipsPerSession: number       // 每会话最大提示数（默认 1）
}
 ...
 * 检查提示是否可以显示
 */
export function canShowTip(tip: Tip): boolean {
 ...
 * 基于优先级和冷却期
 */
export function selectNextTip(): Tip | null {
  // 过滤可显示的提示
 ...
 * 显示提示
 */
export function showTip(): Tip | null {
 ...
 * 注册新提示
 */
export function registerTip(tip: Tip): void {
  state.tips.push(tip)
}
 ...
 * 移除提示
 */
export function removeTip(tipId: string): void {
...
export function getState(): TipState {
  return {
    tips: [...state.tips],
...
export function getHistory(): TipHistory[] {
 ...
 * 构建飞书提示卡片
 */
export function createTipCard(tip: Tip | null): {
  title: string
  content: string
} {
  if (!tip) {
