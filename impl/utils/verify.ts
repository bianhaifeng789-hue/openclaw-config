...
verify('Phase 1: taskTracker exists', () => {
  if (!taskTracker.taskTracker) throw new Error('Missing taskTracker')
})
...
verify('Phase 1: backgroundTaskCard exists', () => {
  if (!backgroundTaskCard.backgroundTaskCard) throw new Error('Missing backgroundTaskCard')
})

// Phase 2: Forked Agent Cache
verify('Phase 2: forkedAgentCache exists', () => {
  if (!forkedAgentCache.forkedAgentCache) throw new Error('Missing forkedAgentCache')
})
...
verify('Phase 2: createCacheParams works', () => {
  ...
  if (!params.systemPrompt) throw new Error('No systemPrompt')
})

// Phase 3: Session Memory Compact
verify('Phase 3: sessionMemoryCompact exists', () => {
  if (!sessionMemoryCompact.compactSessionMemory) throw new Error('Missing compactSessionMemory')
})
...
verify('Phase 4: dangerousPatterns exists', () => {
  if (!dangerousPatterns.dangerousPatterns) throw new Error('Missing dangerousPatterns')
})

verify('Phase 4: detectDangerousPattern works', () => {
  const result = dangerousPatterns.analyzeCommand('rm -rf /')
  if (!result.isDangerous) throw new Error('Should detect rm -rf /')
})

verify('Phase 4: bashCommandClassifier exists', () => {
  if (!bashCommandClassifier.bashCommandClassifier) throw new Error('Missing bashCommandClassifier')
})

verify('Phase 4: classifyCommand works', () => {
  const result = bashCommandClassifier.classifyBashCommand('cat file.txt')
  if (!result.category) throw new Error('No category')
})

verify('Phase 4: permissionDecisionTracker exists', () => {
  if (!permissionDecisionTracker.permissionDecisionTracker) throw new Error('Missing tracker')
})

// Phase 5: Analytics
verify('Phase 5: analyticsService exists', () => {
  if (!analyticsService.analyticsService) throw new Error('Missing analyticsService')
})

verify('Phase 5: sessionTracing exists', () => {
  if (!sessionTracing.sessionTracing) throw new Error('Missing sessionTracing')
})

// Phase 6: Hooks System
verify('Phase 6: hooksSystem exists', () => {
  if (!hooksSystem.hooksSystem) throw new Error('Missing hooksSystem')
})

verify('Phase 6: canUseTool works', () => {
  const result = hooksSystem.canUseTool('read')
  if (typeof result.allowed !== 'boolean') throw new Error('Invalid result')
})

// Phase 7: Swarm Service
verify('Phase 7: swarmService exists', () => {
  if (!swarmService.swarmService) throw new Error('Missing swarmService')
})

verify('Phase 7: createTeammate works', () => {
  const teammate = swarmService.createTeammate('researcher', 'Search')
  if (!teammate.teammateId) throw new Error('No teammateId')
})

// Phase 8: Bridge Service
verify('Phase 8: bridgeService exists', () => {
  if (!bridgeService.bridgeService) throw new Error('Missing bridgeService')
})

verify('Phase 8: checkCapacity works', () => {
  const cap = bridgeService.checkCapacity()
  if (typeof cap.available !== 'boolean') throw new Error('Invalid capacity')
})
