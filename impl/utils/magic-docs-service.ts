...
export interface MagicDoc {
  path: string            // 文件路径
  title: string           // 文档标题
  instructions?: string   // 指令（斜体内容）
  lastUpdateAt: number    // 上次更新时间
  updateCount: number     // 更新次数
  tracked: boolean        // 是否被跟踪
}

export interface MagicDocState {
  trackedDocs: Map<string, MagicDoc>  // 被跟踪的文档
  totalDocs: number                  // 总文档数
  totalUpdates: number               // 总更新次数
  lastScanAt: number                 // 上次扫描时间
}

export interface MagicDocResult {
  success: boolean
  docPath: string
  content?: string
  error?: string
 ...
 */
export function registerMagicDoc(
  filePath: string,
  content: string,
): MagicDoc | null {
 ...
 */
export async function updateAllMagicDocs(
  conversationSummary: string,
): Promise<MagicDocResult[]> {
  const results: MagicDocResult[] = []
...
export function getState(): MagicDocState {
  return {
...
export function getTrackedDocs(): MagicDoc[] {
  return Array.from(state.trackedDocs.values())
}

export function getDoc(path: string): MagicDoc | null {
...
export function createMagicDocsHook(): {
  ...
  runUpdate: () => Promise<MagicDocResult[]>
} {
  return {
    name: 'magic_docs',
    onFileRead: (path: string, content: string) => {
