...
/**
 * Permission Rule Parser Pattern - 权限规则解析器
 * 
 * Source: Claude Code utils/permissions/permissionRuleParser.ts
 * Pattern: parseRule + Allow/Deny + shell wildcard + escape-aware validation + parsePermissionRules
 */

type RuleAction = 'Allow' | 'Deny'

interface PermissionRuleParsed {
  action: RuleAction
  toolPattern: string
  pathPattern: string
  raw: string
...
class PermissionRuleParser {
   ...
   */
  parseRule(line: string, lineNumber?: number): PermissionRuleParsed | null {
   ...
   */
  parsePermissionRules(content: string): PermissionRuleParsed[] {
    const lines = content.split('\n')
    const rules: PermissionRuleParsed[] = []
   ...
   */
  matchTool(toolName: string, toolPattern: string): boolean {
    // Exact match
    if (toolPattern === '*' || toolPattern === toolName) return true
   ...
   */
  matchPath(path: string, pathPattern: string): boolean {
    // Exact match
   ...
   */
  toPermissionConfig(rules: PermissionRuleParsed[]): {
    allow: Array<{ tool: string; path: string }>
    deny: Array<{ tool: string; path: string }>
  } {
    const allow: Array<{ tool: string; path: string }> = []
    const deny: Array<{ tool: string; path: string }> = []
