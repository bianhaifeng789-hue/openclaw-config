 ...
 */
import { runAutoDream, getAutoDreamState, setAutoDreamEnabled } from './auto-dream.js';
import { compactSessionMemory, shouldUseSessionMemoryCompaction, estimateMemoryTokens, setSessionMemoryCompactConfig, getSessionMemoryCompactConfig, } from './session-memory-compact.js';
import { startAgentSummarization, stopAgentSummarization, getAgentSummary, getAllAgentSummaries, } from './agent-summary.js';
// Smart dispatcher and service registry
import { smartDispatch } from './smart-dispatcher.js';
import { autoEnableAll } from './service-registry.js';
import { createAutoDreamCard, createAgentSummaryCard, createTasksSummaryCard, createMemoryCompactCard, cardToJson, } from './feishu-cards.js';
...
function loadHeartbeatState() {
    try {
        ...
        console.error(`[Integration] Failed to load heartbeat state: ${e.message}`);
    }
    return null;
}
/**
...
function saveHeartbeatState(state) {
    try {
        ...
        console.error(`[Integration] Failed to save heartbeat state: ${e.message}`);
    }
}
/**
 ...
 */
export async function hookSessionMemoryCompact(messages) {
    ...
    try {
        ...
        console.error(`[Integration] Failed to load MEMORY.md: ${e.message}`);
    }
    if (!sessionMemory) {
    ...
    console.log(`[Integration] SessionMemoryCompact: ${result.preCompactTokenCount} -> ${result.postCompactTokenCount} tokens`);
 ...
 */
export function hookAgentSummaryStart(taskId, agentId) {
 ...
 */
export function hookAgentSummaryStop(taskId) {
 ...
 */
export async function hookTaskVisualization() {
    ...
    const tasks = summaries.map(s => ({
        ...
        status: (s.running ? 'running' : 'completed'),
    }));
...
export function initIntegrations(config) {
    ...
    const state = loadHeartbeatState();
    if (state) {
        integrationState.autoDreamEnabled = state.autoDream?.enabled !== false;
...
export { 
// AutoDream
runAutoDream, getAutoDreamState, setAutoDreamEnabled, 
// SessionMemoryCompact
compactSessionMemory, shouldUseSessionMemoryCompaction, estimateMemoryTokens, setSessionMemoryCompactConfig, getSessionMemoryCompactConfig, 
// AgentSummary
startAgentSummarization, stopAgentSummarization, getAgentSummary, getAllAgentSummaries, 
// Feishu Cards
createAutoDreamCard, createAgentSummaryCard, createTasksSummaryCard, createMemoryCompactCard, cardToJson, };
/**
 ...
 */
export async function runSmartDispatch(context) {
