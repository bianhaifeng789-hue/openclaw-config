 ...
 * 添加文件到基线（编辑前）
 */
export function beforeFileEdited(filePath, diagnostics) {
 ...
 * 添加新诊断（编辑后）
 */
export function addNewDiagnostics(filePath, diagnostics) {
    const baseline = state.baseline.get(filePath) || [];
    // 找出新增的诊断（不在基线中的）
    const newItems = diagnostics.filter(d => !baseline.some(b => b.message === d.message &&
 ...
 * 生成飞书诊断卡片
 */
export function generateDiagnosticCard(files) {
...
//# sourceMappingURL=diagnostic-tracking-service.js.map