...
export interface InsightsState {
  totalAnalysisRuns: number          // 总分析次数
  lastAnalysisAt: number             // 上次分析时间
  insights: WorkInsight[]            // 洞察列表
  patterns: WorkPattern[]            // 模式列表
  modelUsage: Map<string, number>    // 模型使用统计
  toolUsage: Map<string, number>     // 工具使用统计
...
export interface InsightsConfig {
  enabled: boolean
  analysisIntervalHours: number      // 分析间隔（默认 6h）
  minSessionsForAnalysis: number     // 最小会话数（默认 10）
}
...
export function detectWorkPatterns(
  ...
  // 2. 检测分析模式
  const analysisSessions = sessions.filter(s =>
    s.messages.some(m =>
      m.content.includes('分析') ||
 ...
 * 基于检测到的模式
 */
export function generateInsights(patterns: WorkPattern[]): WorkInsight[] {
...
export function getState(): InsightsState {
  return {
    ...state,
...
export function getConfig(): InsightsConfig {
