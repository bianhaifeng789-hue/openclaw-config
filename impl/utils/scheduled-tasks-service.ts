...
// 定时任务类型
type ScheduledTaskType = 'reminder' | 'recurring' | 'one-time' | 'cron'
...
export function createTask(
  ...
  if (enabled) state.stats.enabledTasks++
...
export function setTaskEnabled(taskId: string, enabled: boolean): boolean {
  ...
  if (task) {
    ...
    if (wasEnabled && !enabled) state.stats.enabledTasks--
    if (!wasEnabled && enabled) state.stats.enabledTasks++
 ...
 * 运行任务（标记）
 */
export function runTask(taskId: string, success: boolean = true): boolean {
  const task = state.tasks.find(t => t.id === taskId)
  if (task) {
    ...
    if (success) state.stats.runsCompleted++
    else state.stats.runsFailed++
 ...
 * 获取统计
 */
export function getStats(): ScheduledState['stats'] {
  return { ...state.stats }
}
...
export function deleteTask(taskId: string): boolean {
  ...
  if (index >= 0) {
    ...
    state.stats.totalTasks--
    if (task.enabled) state.stats.enabledTasks--
    return true
  }
  return false
}
 ...
 * 生成飞书定时任务卡片
 */
export function generateScheduledTasksCard(): object {
