...
export interface AgentSummaryConfig {
  intervalMs: number      // 摘要间隔（默认 30s）
  maxWords: number        // 最大字数（默认 10）
...
export interface AgentSummary {
  ...
  taskId: string          // 任务 ID
  summary: string         // 当前摘要
  previousSummary: string | null  // 上次摘要
...
export interface SummaryState {
  summaries: Map<string, AgentSummary>  // 所有代理摘要
  totalSummaries: number                // 总摘要数
  totalToolUses: number                 // 总工具调用
  averageIntervalMs: number             // 平均间隔
}
...
const DEFAULT_CONFIG: AgentSummaryConfig = {
 ...
 * 获取所有代理摘要
 */
export function getAllSummaries(): AgentSummary[] {
 ...
 * 获取特定代理摘要
 */
export function getAgentSummary(agentId: string): AgentSummary | null {
...
export function getState(): SummaryState {
  return {
...
export function getConfig(): AgentSummaryConfig {
