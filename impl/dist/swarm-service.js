 ...
 * @returns 选中的后端
 */
export function selectBackend(preferredBackend) {
 ...
 */
export function createTeammate(role, task, backendType, model) {
 ...
 */
export async function startTeammate(teammate) {
 ...
 * 完成队友执行
 */
export function completeTeammate(teammateId, result, messages) {
 ...
 * 队友执行失败
 */
export function failTeammate(teammateId, error) {
    const teammate = teammates.find(t => t.teammateId === teammateId);
    if (!teammate) {
        return;
    }
 ...
 */
export function getRunningTeammates() {
    return teammates.filter(t => t.status === 'running');
}
/**
 * Get Teammate Layout
 */
export function getTeammateLayout() {
 ...
 * 创建 Swarm 执行计划
 */
export function createSwarmPlan(mainTask, subtasks, layoutType = 'parallel', maxConcurrency = 3) {
 ...
 * 4. 收集结果
 */
export async function executeSwarmPlan(plan) {
