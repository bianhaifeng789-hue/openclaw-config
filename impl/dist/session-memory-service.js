 ...
 * - 飞书卡片通知
 */
import { getLastCacheSafeParams, runForkedAgent } from './forked-agent-cache';
import { registerTask, updateProgress, completeTask, createTaskCard } from './background-task-utils';
import { updateAutoUpdateBlock, DEFAULT_MEMORY_SECTIONS } from './session-memory-compact';
// 默认配置（借鉴 Claude Code）
...
export function resetSessionMemoryConfig() {
...
export function updateSessionMemoryState(updates) {
    sessionMemoryState = {
        ...sessionMemoryState,
        ...updates
    };
...
export function markExtractionCompleted(summary) {
    updateSessionMemoryState({
        extractionInProgress: false,
 ...
 * 4. 或：上一轮没有工具调用（自然断点）
 */
export function shouldExtractMemory(currentTokenCount, toolCallsSinceLastUpdate, hasToolCallsInLastTurn) {
 ...
 * 5. 发送完成卡片
 */
export async function runMemoryExtraction(messages, currentMemoryContent, sendCard) {
    // 1. 注册任务
    ...
    try {
        ...
        // 7. 解析结果并更新 MEMORY.md
        const updatedContent = updateAutoUpdateBlock(currentMemoryContent, 'current_focus', extractCurrentFocusFromResult(forkResult));
        // 8. 完成任务
 ...
 */
function buildExtractionPrompt(messages, currentMemory) {
    return `
 ...
 */
export async function checkAndExtractMemory(getCurrentTokenCount, getToolCallsCount, hasToolCallsInLastTurn, getCurrentMemory, sendCard) {
    ...
    const result = await runMemoryExtraction([], // 实际需要传入最近消息
    currentMemory, sendCard);
    return {
