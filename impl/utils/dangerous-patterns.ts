...
/**
 * Command Analysis Result - 命令分析结果
 */
 ...
 */
function checkPatternExceptions(
  command: string,
...
export function analyzeCommand(command: string): CommandAnalysisResult {
  ...
  // 计算最高危险等级
  const dangerLevels: DangerLevel[] = matches.map(m => m.dangerLevel)
  const maxDangerLevel = dangerLevels.reduce<DangerLevel>(
    (max, level) => {
 ...
 * 计算风险评分（0-100）
 */
function calculateRiskScore(matches: DangerousPatternMatch[]): number {
  if (matches.length === 0) {
    return 0
  }
  
  // 危险等级权重
  const weights: Record<DangerLevel, number> = {
    safe: 0,
    caution: 10,
    dangerous: 30,
    critical: 50
 ...
 * 快速检查命令是否极危险
 */
export function isCommandCritical(command: string): boolean {
  for (const pattern of DANGEROUS_PATTERNS) {
    if (
      pattern.dangerLevel === 'critical' &&
 ...
 * 获取命令的危险等级
 */
export function getCommandDangerLevel(command: string): DangerLevel {
