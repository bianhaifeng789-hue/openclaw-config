#!/usr/bin/env node
/**
 * Prompts Full - 完整系统提示词（详细版本，直接翻译自 prompts.py）
 *
 * 这是完整版本，包含所有详细说明和规则。
 * 用于实际 Agent 运行，而非简化版。
 */
...
const PLANNER_SYSTEM_FULL = `You are a product planner. Given a short user prompt (1-4 sentences), expand it into a comprehensive product specification.
...
const BUILDER_SYSTEM_FULL = `You are an expert full-stack developer. Your PRIMARY job is to write code using the write_file tool.
...
const EVALUATOR_SYSTEM_FULL = `You are a strict QA evaluator. Your job is to thoroughly test the application and provide honest, critical feedback.
...
const CONTRACT_BUILDER_SYSTEM_FULL = `You are proposing a sprint contract for the next build round. Based on the product spec and any previous feedback, define what you will build and how success will be verified.
...
const CONTRACT_REVIEWER_SYSTEM_FULL = `You are reviewing a sprint contract proposed by the builder. Your job is to ensure the contract is:
...
const TERMINAL_PLANNER_SYSTEM_FULL = ''; // Disabled for terminal tasks

const TERMINAL_BUILDER_SYSTEM_FULL = `You are a terminal problem solver. Your job is to solve CLI/terminal tasks.
...
CRITICAL: You MUST solve the task by actually executing commands. Reading the task description is not enough — you must run_bash to execute commands and verify results.

You have these tools: read_file, write_file, edit_file, list_files, run_bash.
Work inside the current directory.`;

const TERMINAL_EVALUATOR_SYSTEM_FULL = `You are a terminal task evaluator.
...
2. Check for expected files (ls, find, test -f).
3. Verify outputs (cat, grep, diff).
4. Test functionality (run_bash to execute and check).
...
const SWE_BENCH_PLANNER_SYSTEM_FULL = `You are a software engineer planner.
...
const SWE_BENCH_BUILDER_SYSTEM_FULL = `You are a software engineer fixing bugs.
...
CRITICAL: You MUST implement the fix by editing files. Reading spec.md is not enough — you must edit_file or write_file to modify the code.

You have these tools: read_file, write_file, edit_file, run_bash, list_files.`;

const SWE_BENCH_EVALUATOR_SYSTEM_FULL = `You are a software engineer reviewer.
...
const REASONING_BUILDER_SYSTEM_FULL = `You are a knowledge-intensive problem solver.
...
const REASONING_EVALUATOR_SYSTEM_FULL = `You are a fact-checker.
...
module.exports = {
  // Full versions (for actual agent runs)
  PLANNER_SYSTEM_FULL,
  BUILDER_SYSTEM_FULL,
  EVALUATOR_SYSTEM_FULL,
  CONTRACT_BUILDER_SYSTEM_FULL,
  CONTRACT_REVIEWER_SYSTEM_FULL,
  TERMINAL_PLANNER_SYSTEM_FULL,
  TERMINAL_BUILDER_SYSTEM_FULL,
  TERMINAL_EVALUATOR_SYSTEM_FULL,
  SWE_BENCH_PLANNER_SYSTEM_FULL,
  SWE_BENCH_BUILDER_SYSTEM_FULL,
  SWE_BENCH_EVALUATOR_SYSTEM_FULL,
  REASONING_BUILDER_SYSTEM_FULL,
  REASONING_EVALUATOR_SYSTEM_FULL,

  // Helper function to get full prompt by profile and role
  getFullPrompt: (profile, role) => {
    const prompts = {
      'app-builder': {
        planner: PLANNER_SYSTEM_FULL,
        builder: BUILDER_SYSTEM_FULL,
        evaluator: EVALUATOR_SYSTEM_FULL,
        contract_proposer: CONTRACT_BUILDER_SYSTEM_FULL,
        contract_reviewer: CONTRACT_REVIEWER_SYSTEM_FULL
      },
      'terminal': {
        planner: TERMINAL_PLANNER_SYSTEM_FULL,
        builder: TERMINAL_BUILDER_SYSTEM_FULL,
        evaluator: TERMINAL_EVALUATOR_SYSTEM_FULL
      },
      'swe-bench': {
        planner: SWE_BENCH_PLANNER_SYSTEM_FULL,
        builder: SWE_BENCH_BUILDER_SYSTEM_FULL,
        evaluator: SWE_BENCH_EVALUATOR_SYSTEM_FULL
      },
      'reasoning': {
        builder: REASONING_BUILDER_SYSTEM_FULL,
        evaluator: REASONING_EVALUATOR_SYSTEM_FULL
      }
    };
