 ...
 * 记录日志
 */
export function log(level, category, message, metadata) {
    const entry = {
        timestamp: new Date(),
        level,
        category,
    ...
    if (level === 'error')
        state.stats.errorsLogged++;
    if (level === 'warn')
        state.stats.warningsLogged++;
    return entry;
}
/**
 * 快捷方法
 */
export const logDebug = (category, message, metadata) => log('debug', category, message, metadata);
export const logInfo = (category, message, metadata) => log('info', category, message, metadata);
export const logWarn = (category, message, metadata) => log('warn', category, message, metadata);
export const logError = (category, message, metadata) => log('error', category, message, metadata);
/**
 * 获取环境信息
 */
 ...
 * 导出日志到文件
 */
export async function exportLogs(filePath) {
 ...
 * 格式化日志摘要（飞书卡片用）
 */
export function formatLogsSummary(entries) {
    return entries
        .slice(0, 10)
        .map(entry => {
 ...
 * 生成飞书日志卡片
 */
export function generateLogCard() {
...
//# sourceMappingURL=internal-logging-service.js.map