...
export interface DispatcherConfig {
  enabled: boolean
  autoDetect: boolean    // 自动场景检测
  proactiveCall: boolean // 主动调用服务
  minInterval: number    // 最小调用间隔（秒）
  maxCallsPerScene: number // 每场景最大调用数
}

export interface DispatcherStats {
  totalScenesDetected: number
  totalCallsMade: number
  callsByScene: Record<SceneType, number>
...
const SCENE_SIGNALS: Record<SceneType, string[]> = {
  feishu_chat: [
    'channel=feishu',
    'chat_type=direct',
    'chat_type=group',
...
const SCENE_SERVICES: Record<SceneType, ServiceCall[]> = {
  feishu_chat: [
...
let config: DispatcherConfig = DEFAULT_CONFIG
let stats: DispatcherStats = {
  totalScenesDetected: 0,
  totalCallsMade: 0,
  callsByScene: {} as any,
  callsByService: {} as any,
  lastDetection: 0,
  lastCall: 0,
}
...
export function detectScene(context?: string[]): SceneContext {
  const signals = context || detectSignalsFromEnvironment()
  const now = Date.now()
  
  // 计算每个场景的匹配分数
  const sceneScores: Record<SceneType, { score: number; matched: string[] }> = {} as any
  
  for (const [scene, expectedSignals] of Object.entries(SCENE_SIGNALS)) {
    if (expectedSignals.length === 0) continue
    
    const matched = signals.filter(s => 
      expectedSignals.some(expected => 
        s.toLowerCase().includes(expected.toLowerCase())
      )
    )
    
    sceneScores[scene as SceneType] = {
...
function detectSignalsFromEnvironment(): string[] {
  const signals: string[] = []
  
  // 从运行时环境检测
  const runtime = process.env.OPENCLAW_RUNTIME || ''
  if (runtime.includes('feishu')) signals.push('channel=feishu')
  if (runtime.includes('discord')) signals.push('channel=discord')
...
export async function dispatchServices(scene: SceneContext): Promise<ServiceCall[]> {
...
export async function smartDispatch(context?: string[]): Promise<{
  scene: SceneContext
  calls: ServiceCall[]
  summary: string
}> {
...
export function getDispatcherStats(): DispatcherStats & { config: DispatcherConfig; currentScene: SceneContext | null } {
  return { ...stats, config, currentScene }
}

export function setDispatcherConfig(newConfig: Partial<DispatcherConfig>): void {
