...
export interface TaskCardConfig {
  ...
  showResult: boolean        // 显示完成结果
  notifyOnFailure: boolean   // 失败时通知
  compactMode: boolean       // 简洁模式
  updateIntervalMs: number   // 更新间隔
}

export const DEFAULT_CONFIG: TaskCardConfig = {
  showProgress: true,
  showResult: true,
...
// 进度条可视化
function formatProgressBar(progress: number, width: number = 10): string {
 ...
 * 创建任务启动卡片
 */
export function createTaskStartCard(
  task: BackgroundTask,
  config: TaskCardConfig = DEFAULT_CONFIG
 ...
 * 创建任务进度卡片
 */
export function createTaskProgressCard(
  task: BackgroundTask,
  config: TaskCardConfig = DEFAULT_CONFIG
): Record<string, unknown> {
  const taskName = TASK_TYPE_NAMES[task.name] ?? task.name
 ...
 * 创建任务完成卡片
 */
export function createTaskCompleteCard(
  task: BackgroundTask,
  result?: string,
  config: TaskCardConfig = DEFAULT_CONFIG
): Record<string, unknown> {
 ...
 * 创建任务失败卡片
 */
export function createTaskFailedCard(
  task: BackgroundTask,
  config: TaskCardConfig = DEFAULT_CONFIG
): Record<string, unknown> {
  const taskName = TASK_TYPE_NAMES[task.name] ?? task.name
 ...
 */
export function createTasksSummaryCard(
