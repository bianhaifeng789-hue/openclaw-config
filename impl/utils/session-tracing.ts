...
export type TraceEventType =
  ...
  | 'api_response'       // API 响应
  | 'cache_hit'          // Cache 命中
  | 'cache_miss'         // Cache 未命中
  | 'compact'            // 上下文压缩
  | 'memory_extract'     // 记忆提取
  | 'heartbeat'          // 心跳检查
  | 'fork_agent'         // Forked agent
  | 'fork_agent_completed' // Forked agent 完成
...
// 会话追踪存储
const sessionTraces: Map<string, SessionTrace> = new Map()

// 当前追踪链
let currentChain: TraceChain | null = null
 ...
 * @returns 链 ID
 */
export function startTraceChain(sessionId: string): string {
 ...
 * 结束追踪链
 */
export function endTraceChain(): void {
  if (currentChain) {
 ...
 * 获取当前追踪链
 */
export function getCurrentChain(): TraceChain | null {
  return currentChain
}
 ...
 * @returns 子链 ID
 */
export function createChildChain(sessionId: string): string {
 ...
 * 记录带持续时间的事件
 */
export function recordTimedEvent(
  type: TraceEventType,
  name: string,
 ...
 * 结束会话追踪
 */
export function endSessionTrace(sessionId: string): SessionTrace {
  const sessionTrace = sessionTraces.get(sessionId)
 ...
 * 获取会话追踪
 */
export function getSessionTrace(sessionId: string): SessionTrace | undefined {
  return sessionTraces.get(sessionId)
}

// ============================================================================
// Perfetto Format Export
// ============================================================================

/**
 * Perfetto Event - Perfetto 格式事件
 */
 ...
 */
export function exportToPerfetto(sessionTrace: SessionTrace): PerfettoEvent[] {
  const perfettoEvents: PerfettoEvent[] = []
  const pid = 1  // 固定进程 ID
  ...
  for (const chain of sessionTrace.chains) {
    ...
    for (const event of chain.events) {
      const perfettoEvent: PerfettoEvent = {
        name: event.name,
 ...
 * 获取追踪摘要统计
 */
export function getTracingSummary(): TracingSummary {
  ...
  return {
    ...
    avgSessionDurationMs:
      sessions.reduce((sum, s) => sum + (s.totalDurationMs ?? 0), 0) /
      Math.max(1, totalSessions),
    avgApiLatencyMs:
      apiStats.reduce((sum, s) => sum + s.avgLatencyMs * s.requestCount, 0) /
      Math.max(1, apiStats.reduce((sum, s) => sum + s.requestCount, 0)),
    avgToolDurationMs:
      toolStats.reduce((sum, s) => sum + s.avgDurationMs * s.callCount, 0) /
 ...
 * 保存追踪到文件
 */
export function saveTracesToFile(path: string): void {
 ...
 * 从文件加载追踪
 */
export function loadTracesFromFile(path: string): void {
