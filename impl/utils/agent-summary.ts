 ...
 */
const summaryStates = new Map<string, AgentSummaryState>()
 ...
 */
export function stopAgentSummarization(taskId: string): void {
...
function scheduleNextSummary(
  ...
  const timer = setTimeout(
    () => runSummary(taskId, cacheSafeParams),
    SUMMARY_INTERVAL_MS,
  )
...
async function runSummary(
  ...
  try {
    ...
    console.error(`[AgentSummary] Error for ${taskId}: ${(e as Error).message}`)
  } finally {
    state.running = false
    abortControllers.delete(taskId)
    scheduleNextSummary(taskId, cacheSafeParams)
 ...
 */
function notifySummaryUpdate(taskId: string, summary: string): void {
 ...
 */
export function getAgentSummary(taskId: string): string | null {
  const state = summaryStates.get(taskId)
 ...
 */
export function getAllAgentSummaries(): AgentSummaryState[] {
