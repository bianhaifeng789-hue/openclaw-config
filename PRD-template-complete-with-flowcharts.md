...
面向真实项目推进的固定版 PRD 模板。目标不是写一份空泛说明，而是产出一份 **可评审、可设计、可开发、可测试、可验收、可埋点、可协作推进** 的正式产品需求文档。
...
使用原则：
- 这是一个**固定模板**，默认结构不要删，只填空。
- 若某章节本期确实不需要，标记“本期不涉及”，不要直接删除标题。
- 流程图章节为必填项，至少保留“主流程图 + 异常流程图”。
- 每个功能都应至少写清楚：目标、主流程、异常流程、业务规则、验收标准。
...
这一章为固定必填章节，不能删除。至少保留主流程图和异常流程图。
...
- 超过 12 个节点时，拆成主流程、分支流程、异常流程。
- 如果涉及登录态、会员态、审核态、地域态，必须明确分支。
...
## 20. 固定模板使用规范
- 任何新 PRD 默认从本模板复制，不重新发明结构。
- 标题编号固定，方便团队统一评审与检索。
- 若为轻量需求，也至少保留以下章节：3、4、6、8、9、11、16、17。
- 如果没有商业化内容，第 12 章写“本期不涉及”。
- 如果没有复杂配置，第 13 章写“本期仅使用默认配置”。
- 如果没有外部依赖，第 17.2 写“无”。
