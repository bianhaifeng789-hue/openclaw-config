...
export interface ServiceEntry {
  name: string
  file: string
  category: 'core' | 'service' | 'tool' | 'helper' | 'disabled'
  enabled: boolean
  autoStart: boolean  // 是否自动启动（随 heartbeat）
...
export interface RegistryConfig {
  autoEnableCore: boolean    // 自动启用核心服务
  autoEnableServices: boolean // 自动启用所有服务
  autoEnableTools: boolean   // 自动启用工具类（通常 false）
  enableDisabled: boolean    // 启用已禁用的服务
}
...
let registry: Map<string, ServiceEntry> = new Map(
  KNOWN_SERVICES.map(s => [s.name, s])
)
...
export function autoEnableAll(options?: Partial<RegistryConfig>): {
  enabled: string[]
  skipped: string[]
  summary: string
} {
...
export function enableService(name: string): boolean {
...
export function disableService(name: string): boolean {
  const entry = registry.get(name)
  if (entry) {
    entry.enabled = false
...
export function getAutoStartServices(): ServiceEntry[] {
