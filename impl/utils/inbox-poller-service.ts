...
// 收件箱消息类型
type InboxMessageType = 'task' | 'notification' | 'approval' | 'reminder' | 'message'
 ...
 * 添加收件箱消息
 */
export function addMessage(message: InboxMessage): void {
  state.messages.push(message)
  state.stats.totalMessages++
  
  if (message.priority === 'high') state.stats.highPriority++
  if (!message.read) state.stats.unread++
  if (message.actionRequired) state.stats.actionRequired++
}
 ...
 * 获取高优先级消息
 */
export function getHighPriorityMessages(): InboxMessage[] {
 ...
 * 获取需要行动的消息
 */
export function getActionRequiredMessages(): InboxMessage[] {
 ...
 * 模拟轮询（飞书场景）
 */
export function pollInbox(): InboxMessage[] {
 ...
 * 处理消息
 */
export function processMessage(messageId: string): boolean {
  const message = state.messages.find(m => m.id === messageId)
  if (message) {
    message.read = true
    if (message.actionRequired) state.stats.actionRequired--
 ...
 * 获取统计
 */
export function getStats(): PollerState['stats'] {
  return { ...state.stats }
}
 ...
 * 生成飞书收件箱卡片
 */
export function generateInboxCard(): object {
