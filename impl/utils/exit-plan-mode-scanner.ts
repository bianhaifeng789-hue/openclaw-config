...
/**
 * ExitPlanMode Scanner Stateful Classifier Pattern - Plan Mode扫描分类器
 * 
 * Source: Claude Code utils/ultraplan/ccrSession.ts
 * Pattern: SDKMessage ingest + precedence order + rescan after rejection + target tracking
 */

type CCREventType = 'approved' | 'rejected' | 'terminated' | 'pending'
type PlanStatus = 'pending' | 'approved' | 'rejected' | 'terminated'
...
const PRECEDENCE_ORDER: PlanStatus[] = ['approved', 'terminated', 'rejected', 'pending']

class ExitPlanModeScanner {
  private events: CCREvent[] = []
  private targets = new Map<string, TargetState>()
  private lastScanResult: PlanStatus = 'pending'
  private newestApprovedTarget: string | null = null
  private newestNonRejectedTarget: string | null = null

  /**
   * Ingest CCR event stream
   */
  ingest(event: CCREvent): void {
   ...
   */
  private updateTargetState(event: CCREvent): void {
    ...
    const newPrecedence = PRECEDENCE_ORDER.indexOf(event.type as PlanStatus)

    if (newPrecedence < currentPrecedence) {
      target.status = event.type as PlanStatus
   ...
   */
  private rescanAfterRejection(): void {
    let newestTime = 0
   ...
   */
  getNewestNonRejected(): string | null {
