...
function isSwitchableFailure(kind) {
...
export function classifyModelError(error) {
    ...
    if (message.includes('timeout') || message.includes('timed out') || message.includes('abort'))
        return 'timeout';
    if (message.includes('invalid') || message.includes('config') || message.includes('missing baseurl') || message.includes('api key'))
        return 'config';
...
export class ModelRouter {
    ...
    recordFailure(model, kind) {
        ...
        if (kind === 'timeout')
            state.failures[model].timeout += 1;
        if (kind === 'rateLimit')
            state.failures[model].rateLimit += 1;
        if (kind === 'serverError')
            state.failures[model].serverError += 1;
